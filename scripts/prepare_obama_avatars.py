...
AVATAR_DIR = ROOT / "game" / "assets" / "avatars" / "obama"
TARGET_SIDE = 320
STATE_ORDER = ['idle', 'thrust', 'left', 'right', 'fire', 'special', 'victory']
SOURCE_FILES: Dict[str, str] = {
    'idle': 'obamaidle.jpeg',
    'left': 'obamaleft.png',
    'right': 'obamaright.png',
    'fire': 'obama attacking.jpeg',
    'special': 'obamaability.png',
}
...
def fit_canvas(img: Image.Image, side: int = TARGET_SIDE) -> Image.Image:
    ...
    top = max(0, int((height - side) / 2) - int(side * 0.08))
    right = left + side
    bottom = top + side
